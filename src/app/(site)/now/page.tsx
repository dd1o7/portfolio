    ...
    <MasterStack
      master={
        <Pane
          label="~/now"
          counter={
            current ? `${formatDate(current.date)} · ${relativeDate(current.date)}` : undefined
          }
          focused
        >
          {current ? (
      ...
      stack={
        past.length > 0
          ? [
              <Pane key="archive" label="~/now/archive" counter={`${past.length}`}>
